...
unsigned long lastChangedLength1;
...
byte patternLength1;
byte nextBarPatternLength1;
byte currentChosenPatternLength1;
byte pattern1ClockDivider = 1;
bool lengthFlashState = false;
int flashLength = 250;
unsigned long lastFlashedLength;
bool lengthFlashState1 = false;
int flashLength1 = 61;
unsigned long lastFlashedLength1;
bool firstLoadLength = true;
byte prevAnalogue5 = 0;
...
volatile byte beat1 = 0;
volatile byte pattern1ClockDividerCounter = 0;
...
void loop() {
  ...
  bool functionVal = !opt_read(functionPin);
  if(millis() - buttonLastPressed > buttonDebounce){
    ...
    if(functionVal && !lastFunctionState){
  ...
  if(diff >= jitterThreshold){
    ...
    if (p == 0) {  // 8 way switch
      ...
      if(p == 2){
        ...
        if(functionVal && analogueValue != prevAnalogue5){
          nextBarPatternLength1 = analogueValue;
          if(currentChosenPatternLength1 != analogueValue){
            lastChangedLength1 = millis();
          }
          currentChosenPatternLength1 = analogueValue;
        }else if(analogueValue != prevAnalogue5){
        ...
        prevAnalogue5 = analogueValue;
        firstLoadLength = false;
      }else if(p == 6){
        if(analogueValue != prevClockDivideValue){
          pattern1ClockDivider = analogueValue;
          prevClockDivideValue = analogueValue;
        }
      }
    }
  }
...
void advanceBeat() {
  ...
  byte thisGridIndex1 = beat1 / 8;// (maxPatternLength - 1 - beat) / 8;
  byte thisGridBit1 = 7 - (beat1 % 8);//((maxPatternLength - 1 - beat) % 8);

  bool beatChannel = false;
  bool beatChannel1 = false;
  int note = 0;
  bool noteFound = false;
  for (byte br = 0; br < 8; br++) {
    if (!isHalfBeat) {
      beatChannel = bitRead(grid[br][thisGridIndex], thisGridBit);
      beatChannel1 = bitRead(grid[br][thisGridIndex1], thisGridBit1);
  ...
  if (!isHalfBeat) {
    beat++;
    pattern1ClockDividerCounter++;
    if(pattern1ClockDividerCounter >= pattern1ClockDivider){
      pattern1ClockDividerCounter = 0;
      beat1++;
      if(beat1 > patternLength1){
        beat1 = 0;
      }
    }
    if (beat > patternLength) {
      ...
      patternLength1 = nextBarPatternLength1;
      if(loadedPage != page){
        loadPageFromEeprom();
      }
      for (byte r = 0; r < 8; r++) {
...
void refreshDisplay() {
  ...
  if(millis() - lastFlashedLength1 > flashLength1){
    lengthFlashState1 = !lengthFlashState1;
    lastFlashedLength1 = millis();
  }
  bool showLength1 = lengthFlashState1;
  ...
  for (byte r = 0; r < 8; r++) {
    ...
    if (writeDisplayCol == beat
      || (showLength1 && writeDisplayCol == beat1)
      || (showLength && writeDisplayCol == currentChosenPatternLength)
      || (showLength1 && writeDisplayCol == currentChosenPatternLength1)
      || (showRow && r == row)){
      bit = !bit;
    }
